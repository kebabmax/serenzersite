...
- **SV/NO — signalés, non corrigés dans ce lot** : `rit_item4_label` suédois « 7-dagarsutmaning » (mot collé, devrait être « 7-dagars utmaning ») ; norvégien, caractères de soft-hyphen invisibles dans 2 bios (`m_raphael_bio`) et incohérence de casse « organisasjonene »/« Organisasjonene ». À traiter dans un prochain lot.
- **Liens et boutons (agent dédié)** : aucun bouton mort, aucune ancre cassée, sélecteur de langue robuste. Incohérences mineures d'écriture des liens (relatif vs absolu pour Accueil/FAQ selon les pages) relevées mais non bloquantes, non corrigées dans ce lot. Le paramètre `?lang=fr` dans `Contact.html` (lien politique de confidentialité) a été vérifié : **faux positif**, c'est le comportement attendu du gabarit FR par défaut (`i18n.js` utilise bien `?lang=__LANG__` pour les 19 langues générées).
